...
public class MichrosiaProperties {
    ...
    public static AbstractBlock.Properties createLeaves(MaterialColor color) {
        return AbstractBlock.Properties.create(Material.LEAVES, color).harvestTool(ToolType.HOE).notSolid().hardnessAndResistance(0.2f).tickRandomly().sound(SoundType.PLANT).setAllowsSpawn(MichrosiaProperties::allowsSpawnOnLeaves).setSuffocates(MichrosiaProperties::isntSolid).setBlocksVision(MichrosiaProperties::isntSolid);
    }
